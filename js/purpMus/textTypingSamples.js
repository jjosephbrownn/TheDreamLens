...
function setup(){
	...
	let genPrint = sessionStorage.getItem("prevPlace")
	    if(genPrint === "scatter"){
	        ...
	        word = new fracture(0,random([0,1,2]))
	    } else {
	    	word = new fracture(0,random([0,1,2]))
	    }
...
window.addEventListener('keydown', function(e) {
  if(e.keyCode == 32 && e.target == document.body) {
	...
	let bPan1 = map(b,65,90,-1,1)
	let bPan2 = map(b,65,90,1,-1)
	bPan1 = constrain(bPan1, -1,1)
	bPan2 = constrain(bPan2, 1,-1)
	...
	if(structure === 0){
		if(b === 32 && wordState === true){
			word.samplePause(0)
			word.samplePause(1)
			wordState = false
			spacePos++
			word.add(0,map(b,65,90,0,dur1))
			word.add(0,map(b,65,90,0,dur2))
		} else if(b === 32 && wordState === false){
			let r = random([0,1])
			if(r === 0){
				word.samplePlayCont(0,random(-1,1))
				word.delay(0,random(),random(0.5))
			} else if(r === 1){
				word.samplePlayCont(1,random(-1,1))
				word.delay(1,random(),random(0.5))
			}
			wordState = true
			spacePos++
		}
